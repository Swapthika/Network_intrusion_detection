...
def runSVM():
    ...
    # Corrected SelectKBest usage
    selector = SelectKBest(score_func=chi2, k=15)
    X_train1 = selector.fit_transform(X_train, y_train)
    X_test1 = selector.transform(X_test)  # Use transform() only for test data
...
def runANN():
    ...
    X_test = selector.transform(X_test)  # Use transform() only for test data
...
def detectAttack():
    ...
    # Ensure test data has the same feature selection as training
    selector = SelectKBest(score_func=chi2, k=15)
    selector.fit(X_train, y_train)  # Fit selector on training data
    test = test.iloc[:, :X_train.shape[1]]  # Select the same columns as training data
    test = selector.transform(test)  # Transform test data using trained selector
    ...
    for i in range(len(test)):
        if str(y_pred[i]) == '1.0':
            text.insert(END, "X=%s, Predicted=%s" % (test[i], ' Infected. Detected Anamoly Signatures') + "\n\n")
        else:
            text.insert(END, "X=%s, Predicted=%s" % (test[i], 'Normal Signatures') + "\n\n")
